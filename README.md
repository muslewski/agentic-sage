...
(`memory-atlas`, `llm-armory`, `mossferry`). **Off by default.** Nothing is written
...
`repo_id`, `corr`), hashes, and numbers only (exit code, duration in ms). Flags
are allow-listed (`argv_shape`); flag **values** are never kept.

**What is NEVER recorded:** prompts, session transcripts, commit messages, file
contents, absolute or relative paths, environment values, API keys, or free-form
text.
