...
| 2026-07-11 | claude / board (global) | `sage board` from `~/Repositories` (no repo) dead-ends — no home for the global/Hermes "see the whole army" view | med — blocks the Hermes use case | FIXED (plan 018, `ed50e27`): `sage war` = live cross-repo cockpit (panels + grouped body + scroll) + `sage.war --json` envelope; `sage board` outside a repo now hints at it. Verified live: `sage war --json` reads 4 repos / 28 sessions |
