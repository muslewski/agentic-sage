...
test('fleet prints the nearest-neighbour line; board renders with tmux best-effort', () => {
  ...
  const repo = mkGitRepo()
  const id = resolveRepoId(repo)
  seedSession(home, id, { session_id: 'a', branch: 'feat-a', touched_globs: ['src/a.ts'], liveness: 'idle', updated_at: '2026-06-28T11:00:00Z', pid: 1 })
  seedSession(home, id, { session_id: 'b', branch: 'feat-b', touched_globs: ['src/b.ts'], liveness: 'idle', updated_at: '2026-06-28T12:00:00Z', pid: 1 })
...
test('backlog: no adapter → clean line; with adapter → row board', () => {
  ...
  const repo = mkGitRepo()
  const id = resolveRepoId(repo)
  seedSession(home, id, { session_id: 's1', branch: 'feat-x', updated_at: '2026-06-28T12:00:00Z', pid: 1 })
