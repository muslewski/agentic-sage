...
export const collectSessions = (home, repoId, now) => {
  ...
  for (const f of files) {
    ...
    const liveness = deriveLiveness({ alive, closed, lastToolAt: rec.last_tool_at, phase: rec.phase, now })
    const { bucket, age } = handoffBucket(rec.handoff_at, now)
    const outRec = { session_id: sid, ...rec, alive, liveness, handoff_bucket: bucket, handoff_age: age }
...
export const renderBoard = (sessions, { repoId, wide } = {}) => {
  ...
  const lines = rows.map((r) => {
    ...
    return `● ${padR(r.id, wId)}  ${padR(r.status, wSt)}  ${padR(r.zone, wZo)}  ${padR(r.when, wWh)}${tail ? `  ${tail}` : ''}`.replace(
      /\s+$/,
      '',
    )
  })
...
// spinner frame. renderBoard emits head, '', row, row… so line i maps to
// sessions[i - 2]; only `working` rows (the session is actively doing something)
// spin — everything else keeps its static ●.
export const spinnerize = (text, sessions, frame) =>
  text
    .split('\n')
    .map((ln, i) => {
      const s = sessions[i - 2]
      const busy = s && (s.liveness === 'working' || s.phase === 'compacting')
      return busy && ln.startsWith('● ') ? frame + ln.slice(1) : ln
    })
    .join('\n')
