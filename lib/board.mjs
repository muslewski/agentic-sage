...
import { collectSyntheticSessions, mergeSynthetic } from './agent-status.mjs'
import { rollupSessions } from './rollup.mjs'
...
const collectRealSessions = (home, repoId, now, { dir: dirArg } = {}) => {
  ...
  for (const f of files) {
    ...
    try {
      ...
      // Regular files only: FIFO/socket under sessions/ would hang readFile forever.
...
// Hermetic default: when `home` is a temp test home (not the real homedir), do
// not read the host's Agent Status Provider dir — that would leak 100s of live
// children into unit tests and break exact counts.
// Use passwd homedir, NOT os.homedir()/\$HOME: unit tests set HOME to a
// temp dir, which makes os.homedir() equal the temp path and would otherwise
// leak the host's 300+ agent-status records into hermetic fleets.
const realUserHome = () => {
  try {
...
const resolveAgentStatusDir = (home, opts = {}) => {
  if (opts.agentStatusDir !== undefined) return opts.agentStatusDir
  if (opts.noSynthetic) return path.join(String(home || ''), '__no_agent_status__')
  try {
    if (!home || path.resolve(home) !== path.resolve(realUserHome())) {
      return path.join(String(home || ''), '__no_agent_status__')
    }
  } catch {
    return path.join(String(home || ''), '__no_agent_status__')
  }
...
// Synthetic rows are merged OUTSIDE the repoCache: the agent-status dir and
// the sessions dir have independent mtimes, so caching the merge would pin a
// stale child list behind a fresh session list.
export const collectSessions = (home, repoId, now, opts = {}) => {
  const real = collectRealSessions(home, repoId, now, opts)
  if (opts.noSynthetic) return real
  let syn = []
  try {
    syn = collectSyntheticSessions(home, repoId, now, {
      dir: resolveAgentStatusDir(home, opts),
    })
  } catch {
    syn = [] // observer discipline: never let the bridge break the board
  }
  return mergeSynthetic(real, syn)
}
...
const rowFields = (s, { wide, zoneWidth } = {}) => {
  ...
  const zone = Number.isFinite(zoneWidth) && zoneWidth > 0 ? clipZone(zoneRaw, zoneWidth) : zoneRaw
  return {
...
export const renderBoard = (sessions, { repoId, wide, all = false, flat = false, cols } = {}) => {
  ...
  // --all and --flat both disable rollup: the human asked for everything, or
  // explicitly for a peer list. Default path collapses past the viewport budget.
  const doRollup = !all && !flat
  const rolled = doRollup ? rollupSessions(live) : { rows: live, groups: [] }
  const shownLive = rolled.rows
  const shown = fold ? shownLive : shownLive.concat(archive)
  ...
  const groupLines = (rolled.groups || []).map((g) => {
    const sample = (g.sample || [])[0] || ''
    const more = g.count > 1 ? ` +${g.count - 1}` : ''
    return `  ▸ ${g.label.padEnd(22)} ${g.live} live · ${g.dead} dead   (${sample}${more})`
  })
  ...
  const lines = fold
    ...
    : [head, '', ...(groupLines.length ? [...groupLines, ''] : []), ...body]
