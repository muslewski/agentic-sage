...
export const tally = (rows) => {
  ...
  return { live, working, nested, human: rows.length - nested }
}
...
export const collectFleet = (home, now = Date.now()) => {
  const repos = []
  const totals = { repos: 0, sessions: 0, live: 0, working: 0, contested: 0, human: 0, nested: 0 }
  for (const { repoId } of listRepos(home)) {
    ...
    const { live, working, nested, human } = tally(rows)
    let contested = 0
    try {
    ...
    repos.push({ repoId, label: stripHash(repoId), sessions: rows, live, working, human, nested })
