...
// `working` counts liveness==='working' (includes compacting, which deriveLiveness
// maps to 'working' for hotness/collision awareness). `compacting` is the explicit
// subset for richer display (additive, zero-herald safe).
export const tally = (rows) => {
  ...
  const nested = rows.filter(isNested).length
  const compacting = rows.filter((s) => s.phase === 'compacting').length
  return { live, working, nested, human: rows.length - nested, compacting }
}
...
export const collectFleet = (home, now = Date.now()) => {
  const repos = []
  const totals = { repos: 0, sessions: 0, live: 0, working: 0, contested: 0, human: 0, nested: 0, compacting: 0 }
  for (const { repoId } of listRepos(home)) {
    ...
    repos.push({ repoId, label: stripHash(repoId), sessions: rows, live, working, human, nested, compacting })
