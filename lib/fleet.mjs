...
export const fleetLine = (sessions, { selfSid } = {}) => {
  const others = sessions
    .filter((s) => s.session_id !== selfSid && !DEAD.has(s.liveness) && s.role !== 'judge')
...
export const contestedCount = (rows = []) => {
  ...
  for (const s of rows) {
    if (!LIVE.has(s?.liveness) || s?.role === 'judge') continue
...
// Same hermetic rule as board.collectSessions: temp homes must not read the
// host agent-status dir (would invent repos mid-test).
const realUserHome = () => {
  try {
...
const fleetAgentStatusDir = (home, opts = {}) => {
...
export const collectFleet = (home, now = Date.now(), opts = {}) => {
  const repos = []
  const totals = {
    repos: 0,
    sessions: 0,
  ...
  for (const { repoId } of listRepos(home)) {
    let rows = []
    try {
      rows = collectSessions(home, repoId, now, opts)
    } catch {
      rows = []
    }
    seen.add(repoId)
    ...
    const judges = rows.filter((s) => LIVE.has(s.liveness) && s.role === 'judge').length
    repos.push({
      repoId,
    ...
    totals.judges += judges
  }
  // Children whose cwd resolves to a repo SAGE has never stored (a fresh clone,
  // or the fleet's --cwd parent) would otherwise vanish. Group them under their
  // resolved repo id so `sage fleet` shows the repo appearing, not nothing.
  // collectSessions already merged per known repo; only add unclaimed remainder.
  if (!opts.noSynthetic) {
    let allSyn = []
    try {
      allSyn = collectSyntheticSessions(home, null, now, { dir: fleetAgentStatusDir(home, opts) })
    } catch {
      allSyn = []
    }
    ...
    for (const s of allSyn) {
      const id = s.repo_id
      if (!id || seen.has(id)) continue // known repos already merged via collectSessions
      if (!byRepo.has(id)) byRepo.set(id, [])
      byRepo.get(id).push(s)
    }
    for (const [repoId, syn] of byRepo) {
      const rows = mergeSynthetic([], syn)
      const { live, working, nested, human, compacting } = tally(rows)
      const contested = contestedCount(rows)
      const judges = rows.filter((s) => LIVE.has(s.liveness) && s.role === 'judge').length
      repos.push({
...
export const activitySpark = (
  timestamps = [],
  { now = Date.now(), buckets = 8, windowMs = 24 * 3600000 } = {},
) => {
...
export const buildReposView = (fleet, { now = Date.now() } = {}) => {
  ...
  const rank = (a, b) =>
    b.live - a.live || b.sessions - a.sessions || a.label.localeCompare(b.label)
