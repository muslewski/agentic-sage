...
import { packageVersion, readInstallState, writeInstallState, statePath } from './install-state.mjs'
...
export function computePackageFreshness(home, opts = {}) {
  ...
  if (wantRegistry) {
    ...
    if (!opts.forceRegistry && cacheAge < ttlMs && cached?.latest) {
      ...
      if (latest) {
        // Cache only when sage home already exists. Never mkdir the home from a
        // freshness probe — that would contradict "sage home missing" from gate.
        const homeDir = path.dirname(statePath(home))
