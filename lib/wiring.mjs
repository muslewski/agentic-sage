...
import { stampWired } from './install-state.mjs'
import { stabilizePackageRoot } from './user-scope-wiring.mjs'
...
export function wireAll({
  ...
  // Never symlink hooks/skills into a linked worktree — worktree cleanup
  // dangles every Stop hook. Prefer the main checkout when detectable.
  repoRoot = stabilizePackageRoot(repoRoot)
  ...
  // Stamp install freshness Tier A (package version now wired).
  try {
    stampWired(home)
  } catch {
    /* never fail wire */
...
export function wireProject({
  ...
  // Package root for emitter/skill symlinks: never a worktree path.
  repoRoot = stabilizePackageRoot(repoRoot)
...
function _mergeSettings(settingsPath, hookLink, nodeExecPath, { scopeFlag, staleLink } = {}) {
  ...
  // Our hooks are identified by the emitter link path (stable across node
  // versions) — the current hookLink OR a pre-rename staleLink. Drop every one
  // of OURS, then push exactly one fresh entry. This dedups a re-init under a
  // different node path (nvm switch) that a full-command-string check would miss
  // and let double-fire. Foreign hooks (command lacks our link) are untouched.
  const ours = (h) =>
...
// working shape (N events incl. PostCompact, each an array containing one {hooks: [{type,command}]}).
