...
// Both install.mjs and `sage init` call wireAll(). wireProject() is the
// project-scoped counterpart (plan 009) — separate export so wireAll's
// global contract stays untouched.
...
export function wireProject({
  ...
  // later step throws, so it's safe to do before the settings write).
  const hooksDir = profile.hooksDir(home)
  fs.mkdirSync(hooksDir, { recursive: true })
  const link = path.join(hooksDir, 'sage-emit.mjs')
  const target = path.join(repoRoot, 'hooks', 'sage-emit.mjs')
  ...
  // an aborted install never half-writes project state.
  const settingsPath = profile.projectSettings(mainRoot)
  _mergeSettings(settingsPath, link, nodeExecPath, { scopeFlag: '--scope=project' })
...
function _mergeSettings(settingsPath, hookLink, nodeExecPath, { scopeFlag } = {}) {
