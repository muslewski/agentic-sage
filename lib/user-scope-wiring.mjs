    ...
    for (const grp of groups) {
      ...
      for (const h of list) {
        ...
        for (const raw of extractCommandPaths(h.command)) {
          ...
          // Interpreters (…/bin/node) are version-manager noise on every nvm
          // desk — only package/script paths count for latent-risk labels.
          // Missing interpreter still surfaces as wired-missing.
          ...
          if (isProbablyInterpreter(resolved)) continue
          // Latent: non-interpreter command path exists but is fragile.
...
export function extractCommandPaths(command) {
  ...
  // Permission-decision hooks sometimes use a raw JSON object as `command` —
  // never treat that blob as a filesystem path.
  ...
  while ((m = re.exec(command)) !== null) {
    ...
    // Only absolute or home-relative paths — not free-form prose with slashes.
    if (tok.startsWith('/') || tok.startsWith('~/') || tok === '~') out.push(tok)
  }
  return out
}
...
function pathLooksLikeFileRef(p) {
  ...
  // Absolute or home-relative only (skip bare "node").
  return p.startsWith('/') || p.startsWith('~')
}

function isProbablyInterpreter(p) {
