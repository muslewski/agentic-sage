...
// Root + id in ONE git spawn — the emitter needs both; resolving them
// separately doubles the subprocess cost on PreCompact / armed PreToolUse.
export const resolveRepo = (cwd) => {
  const root = resolveRepoRoot(cwd)
  return root ? { root, id: repoIdFromRoot(root) } : null
}
