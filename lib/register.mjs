...
//
// Soft-fail returns { ok: false }. CLI maps: 0=success, 1=soft-fail, 2=usage.
// A registration failure must never throw into a launcher's uncaught path
// (except missing --sid TypeError for programmers).
import { mergeRecord, readRecord, isSafeSessionId } from './store.mjs'
...
export const registerSession = ({
  ...
  if (!isSafeSessionId(sid)) return { ok: false, sid, reason: 'unsafe session id' }
  const r = resolveOrFail(cwd)
  if (!r.ok) return { ok: false, sid, reason: r.reason }
  const { id: repoId, worktree } = r.repo
  const ts = iso(now)
  const patch = omitUndef({
    session_id: sid,
    repo_id: repoId,
    // Actual checkout of this session, not the main-repo root — outer vs
    // nested-inner must stay distinct worktree paths.
