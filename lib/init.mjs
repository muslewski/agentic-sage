...
import { MARKER_DIR, registryPath, explainRepoDataDir, sageHome } from './roots.mjs'
...
export async function runWizard({ input, output, cwd, defaults = {} }) {
  ...
  try {
    ...
    const harnessIdx = await choose(rl, output, 'Harness', harnessNames, 0)
    const harness = harnessNames[harnessIdx]
    ...
    if (scope === 'project') {
      ...
      const storageIdx = await choose(
        ...
        output,
        'Storage',
        ['~/.claude/sage (default)', 'custom path'],
        0,
      )
      if (storageIdx === 1) {
 ...
 * @param {{scope:'global'|'project', enabled:boolean, dataDir:string, home:string}} opts
 */
export function renderSummary({ scope, enabled, dataDir, home }) {
 ...
 */
export function renderShow({ home, cwd }) {
  const profile = getHarness('claude')
  ...
  const hooksLink = path.join(profile.hooksDir(home), 'sage-emit.mjs')
  let hookTarget = 'not linked'
  try {
    hookTarget = fs.readlinkSync(hooksLink)
  } catch {
    /* not linked */
  }
  ...
  return [
    ...
    '  Harness (claude)',
    `    settings   ${profile.settings(home)}`,
