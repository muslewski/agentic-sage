...
function emitFleetDevlog(args) {
  try {
    ...
    // Contract repo_id (basename-sha2568 of main root) — leave legacy hashRepoRoot alone.
    const rid = resolveRepoId(args.repoRoot || process.cwd())
...
export function trackCommand(args) {
  // Fleet stream uses its own enable gate; always attempt (except self-cmds).
  if (args.cmd !== 'telemetry' && args.cmd !== 'statusline') emitFleetDevlog(args)
