...
// nameMode cycles with `t`: name (default) | branch | both
// showZone defaults false — zone is optional localization (toggle `z`).
export const NAME_MODES = ['name', 'branch', 'both']
export const nextNameMode = (cur) => {
  const i = NAME_MODES.indexOf(cur)
  return NAME_MODES[(i < 0 ? 0 : i + 1) % NAME_MODES.length]
}

// Responsive column layout. CRITICAL: rowW MUST be ≤ terminal cols.
// Zone is OFF by default so SESSION/NAME gets the width (intuitive identity).
export const layoutFor = (cols, { showZone = false } = {}) => {
  const term = Number.isFinite(cols) && cols >= 40 ? Math.floor(cols) : 80
  const rules = showZone ? 3 : 2 // │ separators between cells
  ...
  if (showZone) {
    const zone = Math.max(8, Math.min(14, Math.floor(pool * 0.28)))
    const id = Math.max(12, pool - zone)
    return { id, status, zone, when, meta, rowW: fixed + id + status + zone + when + meta, showZone: true, term }
  }
  // No zone — give the spare width to the name column.
  const id = Math.max(16, pool)
  return { id, status, zone: 0, when, meta, rowW: fixed + id + status + when + meta, showZone: false, term }
}
// Defaults for tests / non-TTY (80-col, zone off = name-first baseline).
...
// Human identity for the SESSION column.
// name  → tmux window name if set, else branch  (default — what the human named it)
// branch → git branch only
// both  → "window · branch" when both exist
export const sessionName = (s, nameMode = 'name') => {
  const branch = s?.branch || '(none)'
  const win = (s?.window_name || '').trim()
  if (nameMode === 'branch') return branch
  if (nameMode === 'both') return win ? `${win} · ${branch}` : branch
  return win || branch
}

// One session row. Default: NAME | STATUS | AGE (zone hidden).
// `nameMode` / `showZone` control identity + localization columns.
export const sessionRow = (s, { wide, cols, nameMode = 'name', showZone = false } = {}) => {
  const L = layoutFor(cols, { showZone })
  const isHot = s.liveness === 'working' || s.phase === 'compacting'
  const lead = isHot ? '◆' : '●'
  const id = `${sessionName(s, nameMode)}${s.dirty ? ' ✎' : ''}`
  const pct =
...
export const renderRepoSection = (
  repo,
  { wide, showNested = false, cols, nameMode = 'name', showZone = false } = {},
) => {
  const all = repo.sessions || []
  ...
  return [head, ...sessions.map((s) => sessionRow(s, { wide, cols, nameMode, showZone }))]
}
...
export const bodyModel = (
  ...
  (fleet.repos || []).flatMap((r) => {
    ...
    const [head, ...rows] = renderRepoSection(r, { wide, showNested, cols, nameMode, showZone })
    return [
...
export const HELP_KEYS = [
  ...
  ['t', 'cycle name column: name → branch → both'],
  ['z', 'toggle ZONE column (path localization)'],
...
export const footer = (
  ...
  const nm = nameMode === 'branch' ? 'branch' : nameMode === 'both' ? 'both' : 'name'
  // Reserve the always-visible tail so "help" / "quit" never get sliced off.
  const tail = '? help · q quit'
  const tailW = [...` · ${tail}`].length
  const budget = Math.max(20, width - tailW)
  // Head chips in priority order — pack until budget full (words stay attached).
  const head = [
    '↑↓ move',
    '↵ open',
    '/ filter',
    ...
    showAll ? 'all✓' : 'all',
    deadCount > 0 && showAll ? `X clear×${deadCount}` : deadCount > 0 ? 'manage' : null,
  ].filter(Boolean)
  let packed = ''
...
// Column-label header — NAME (or BRANCH) | STATUS | [ZONE] | AGE
export const columnHeader = (cols, { nameMode = 'name', showZone = false } = {}) => {
  const L = layoutFor(cols, { showZone })
  const nameLabel = nameMode === 'branch' ? 'BRANCH' : nameMode === 'both' ? 'SESSION' : 'NAME'
...
export const renderWarRoom = (
  ...
  const model =
    prebuiltModel || bodyModel(fleet, { wide, showNested, cols: termCols, nameMode, showZone })
  ...
  return [
    header,
    panels,
    columnHeader(termCols, { nameMode, showZone }),
    ...body,
