...
export const renderPanels = (totals = {}, heatValues = []) => {
  // Layer B (plan 024): headline human-managed + nested; live/hot stay secondary.
  const t = {
    repos: 0,
    sessions: 0,
    live: 0,
  ...
  const human = t.human || Math.max(0, (t.sessions || 0) - (t.nested || 0))
...
// rollup. Layer B (plan 024): by default list only human-managed rows; fold
// nested armory children into the band (`· +N nested`) until `showNested`.
export const renderRepoSection = (repo, { wide, showNested = false } = {}) => {
  ...
  const sessions = visibleSessions(repo, { showNested })
...
export const bodyModel = (fleet, { wide, showNested = false } = {}) =>
  (fleet.repos || []).flatMap((r) => {
    const sessions = visibleSessions(r, { showNested })
