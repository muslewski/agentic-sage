...
export const bodyModel = (fleet, { wide } = {}) =>
  (fleet.repos || []).flatMap((r) => {
    ...
    return [
      { text: head, header: head, isHeader: true },
      ...rows.map((t) => ({ text: t, header: head, isHeader: false })),
    ]
  })
...
// Sticky-header viewport over an attributed model. When the top visible line is
// a session row whose repo band has scrolled off above, PIN that band as line 0
// — it overlays the row sliding under it, so the region stays exactly `height`
// lines and you never lose track of which repo the rows belong to.
export const stickyViewport = (model, { scroll = 0, height } = {}) => {
  if (!Number.isFinite(height) || height >= model.length)
    return { lines: model.map((m) => m.text), scroll: 0, maxScroll: 0, stuck: null }
  const maxScroll = Math.max(0, model.length - height)
  const s = Math.max(0, Math.min(scroll, maxScroll))
  const window = model.slice(s, s + height)
  const top = window[0]
  if (top && !top.isHeader) {
    return {
      lines: [top.header, ...window.slice(1).map((m) => m.text)],
      scroll: s,
      maxScroll,
      stuck: top.header,
    }
  }
  return { lines: window.map((m) => m.text), scroll: s, maxScroll, stuck: null }
}
...
export const renderWarRoom = (
  ...
  const vp = stickyViewport(model, { scroll, height })
  return [header, panels, ...vp.lines, footer(showAll, vp.scroll, vp.maxScroll)].join('\n')
}
