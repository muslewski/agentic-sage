...
export const renderPanels = (totals = {}, heatValues = []) => {
  ...
  // Judge chrome: live judge sessions and/or fresh briefs (incl. grace).
  const judgeLine =
    t.judge_label ||
    (t.judges > 0
      ? `⚖ ${t.judges} judge${t.judges === 1 ? '' : 's'}`
      : t.judge_grace
        ? '⚖ grace'
        : '')
  const fleet = box(
    'FLEET',
    [`${t.repos} repos`, judgeLine || `${human} human`],
    W_FLEET,
  )
  const active = box(
    'ACTIVE',
    [`${t.live} live`, judgeLine ? `${human} human` : `${t.nested || 0} nested`],
    W_ACTIVE,
  )
  const heatBits = []
  if (t.compacting > 0) heatBits.push(`${t.compacting} compact`)
  if (t.contested > 0) heatBits.push(`${t.contested} ⚔`)
  if (t.judges > 0 && !judgeLine) heatBits.push(`⚖${t.judges}`)
