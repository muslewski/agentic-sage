...
// Default grid: NAME | BRANCH | STATUS | AGE — balanced, no NAME desert.
// ZONE is optional (`z`). showZone defaults false.
...
// One session row. Default: NAME | BRANCH | STATUS | AGE.
// Zone optional. Dirty mark sits on NAME (or BRANCH if NAME is placeholder).
export const sessionRow = (s, { wide, cols, showZone = false } = {}) => {
...
export const renderRepoSection = (
  repo,
  { wide, showNested = false, cols, showZone = false } = {},
) => {
  const all = repo.sessions || []
...
export const bodyModel = (fleet, { wide, showNested = false, cols, showZone = false } = {}) =>
  (fleet.repos || []).flatMap((r) => {
    const { sessions } = prepareRepoView(r, { showNested })
    const [head, ...rows] = renderRepoSection(r, { wide, showNested, cols, showZone })
    return [
...
// Column-label header — NAME | BRANCH | STATUS | [ZONE] | AGE
export const columnHeader = (cols, { showZone = false } = {}) => {
  const L = layoutFor(cols, { showZone })
