...
export const footer = (
  ...
  scroll = 0,
  maxScroll = 0,
  { mode = 'nav', query = '', workingOnly = false } = {},
) => {
  ...
  return ` ↵ enter · ↑↓ move · / filter · w ${wf} · c cd · a ${showAll ? 'dead✓' : 'all'} · q quit${pos}`
}
...
export const renderWarRoom = (
  ...
  return [
    ...
    panels,
    ...body,
    footer(showAll, vp.scroll, vp.maxScroll, { mode, query, workingOnly }),
  ].join('\n')
}
