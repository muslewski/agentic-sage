...
export const renderPanels = (totals = {}, heatValues = []) => {
  ...
  if (t._memory) {
    ...
    const active = box('ARCHIVE', [`${dead} records`, 'terminal'], W_ACTIVE)
    const heat = box('CLEAN', ['X clear', 'm manage'], W_HEAT)
    return fleet.map((ln, i) => ln + active[i] + heat[i]).join('\n')
  }
...
export const footer = (
  ...
  let head
  if (face === 'clash') {
    head = ['↑↓ move', '↵ open', '/ filter', workingOnly ? 'work✓' : 'work', 'clear=good'].filter(Boolean)
  } else if (face === 'memory') {
    head = [
      '↑↓ move',
