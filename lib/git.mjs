...
// Nested git worktree / nested checkout dirs appear as untracked `?? nested/`
// in the outer worktree's porcelain. Recording them as outer's touched product
// invents false contested territory. Skip paths that are themselves a git root.
const isNestedGitCheckout = (worktree, rel) => {
  if (!rel || rel.includes('\0')) return false
  // Only top-level or directory-looking untracked entries matter; a file is fine.
...
export const gitSignals = (worktree, { trunk: trunkHint } = {}) => {
  let head = null
  let porcelain = ''
  let trunk = typeof trunkHint === 'string' && trunkHint ? trunkHint : null
  ...
  try {
    const t = trunk || trunkOf(worktree)
    const names = git(worktree, ['diff', '--name-only', '-z', `${t}...HEAD`])
    // Only commit `t` to the outer `trunk` once the diff actually succeeded —
    // trunkOf() always falls back to a guessed 'main' and never throws, so a
    // non-repo (or a bad hint) must not leak that guess into the return value.
    trunk = t
  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    const p = e.slice(3)
    // Untracked nested worktree checkout (?? nested-inner/) — not outer product.
    if (e[0] === '?' && e[1] === '?' && isNestedGitCheckout(worktree, p.replace(/\/$/, ''))) {
      continue
    }
    touched.add(p)
  ...
  return { head, dirty: porcelain.length > 0, touched: [...touched], trunk }
}
