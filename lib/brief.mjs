...
export const normalizeBrief = (input, { now = Date.now() } = {}) => {
  ...
  const hotspots = Array.isArray(input.hotspots)
    ...
    : []
  // Per-session one-liners for ferry / about (auto from fact judge run or publish).
  const session_lines = Array.isArray(input.session_lines)
    ? input.session_lines
        ...
        .map((row) => ({
          ...
          // Topic fingerprint (branch|window|claims) — about drops line if live topic moved
          fingerprint:
            row.fingerprint != null && String(row.fingerprint)
              ? String(row.fingerprint).slice(0, 500)
              : undefined,
        }))
        .slice(0, 80)
    : []
