...
export const toSyntheticSession = (
  ...
  // repo id (lib/repo-id.mjs:24-32), which is what we want. Always resolve so
  // collectFleet can bucket unclaimed children by their own repo_id.
  let recRepo = null
  try {
  ...
  if (repoId && recRepo !== repoId) return null
...
// Real records win on pid collision. Never field-merge the two: a synthetic
// row's empty touched_globs means "unknown", not "touched nothing", and
// blending the two would let the collision surface believe a lie.
export const mergeSynthetic = (real = [], synthetic = []) => {
  if (!synthetic.length) return real
  const pids = new Set(real.map((r) => r.pid).filter(Number.isFinite))
  const extra = synthetic.filter((s) => !pids.has(s.pid))
  if (!extra.length) return real
  return [...real, ...extra].sort((a, b) =>
