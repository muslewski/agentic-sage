...
// Path-segment boundary for both-glob static prefixes.
// `nested/**` must NOT collide with `nested-inner/**` — plain startsWith would
// treat "nested-inner" as under "nested". Empty prefix (e.g. `*.ts`) is vacuous.
const prefixCompatible = (pa, pb) => {
  if (pa === pb) return true
  if (!pa || !pb) return true
  if (pa.startsWith(pb)) return pa.length === pb.length || pa[pb.length] === '/'
  if (pb.startsWith(pa)) return pb.length === pa.length || pb[pa.length] === '/'
  return false
}
...
export const overlaps = (a, b) => {
  ...
  if (!prefixCompatible(pa, pb)) return false
...
export const claimsOf = (rec) => ({
  claimed: Array.isArray(rec?.claimed_globs) ? rec.claimed_globs : [],
  touched: Array.isArray(rec?.touched_globs) ? rec.touched_globs : [],
})
...
// Live workers only — judge sessions reason at fleet altitude and must not
// appear as collision peers (they do not claim product territory).
// Synthetic agent-status rows have no verified touched_globs — including them
// would assert "this child touched nothing" and give a false all-clear on the
// exact surface that exists to prevent collisions. Real records only.
const others = (home, repoId, now, selfSid) =>
  collectSessions(home, repoId, now).filter(
    (s) => s.session_id !== selfSid && LIVE.has(s.liveness) && s.role !== 'judge' && !s.synthetic,
  )
...
// Every path two or more sessions have touched *or claimed* — the conflict
// surface. claimed_globs alone must still surface as contested (territory and
// why-diverged already union both; merge-brief must match).
