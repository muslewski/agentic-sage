...
export const mergeBrief = (home, repoId, { now, selfSid } = {}) => {
  ...
  for (const [p, sessions] of byPath) {
    ...
    out.push({ path: p, generated: isGenerated(p), sessions })
  }
...
export const renderTerritory = (overlapped, { queries } = {}) => {
  ...
  const rows = overlapped.map(
    (o) =>
      `  ${pad(o.branch || o.session_id, 18)} ${pad(o.via, 8)} ${pad(o.liveness, 8)} ${o.hit}${o.generated ? '  [generated]' : ''}`,
  )
...
export const renderWhyDiverged = (touches, { file } = {}) => {
  ...
  const rows = touches.map((t) => {
    const stat = t.stat && t.stat.length ? `  +${t.stat[0].added}/-${t.stat[0].deleted}` : ''
    return `  ${pad(t.branch || t.session_id, 18)} ${pad(t.via, 8)} ${pad(t.liveness, 8)}${stat}`
  })
