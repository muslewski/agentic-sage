...
// Full header line: brand + optional judge chip + tabs + clock, ≤ cols.
// `counts.judgeChip` is a short string like "⚖ 1 live · fleet" (optional).
export const renderWarHeader = (face, clock = '', cols = 80, counts = null) => {
  const w = Number.isFinite(cols) && cols >= 40 ? Math.floor(cols) : 80
  const chip =
    counts?.judgeChip && String(counts.judgeChip).trim()
      ? ` ${String(counts.judgeChip).trim()}`
      : ''
  const brand = `⚔  SAGE WAR${chip}`
  // ⚔ is 2 display cols in most terms
  const brandCols = [...brand].length + 1
  let mid = renderTabMid(face, counts)
  const clockS = clock || ''
  const fits = (m) => brandCols + 2 + [...m].length + 2 + clockS.length <= w
  if (!fits(mid)) {
    // Drop chip first to free width
    if (chip) {
      return renderWarHeader(face, clock, cols, { ...counts, judgeChip: '' })
    }
