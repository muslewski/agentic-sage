...
export const contestedPaths = (rows = []) => {
  ...
  for (const s of rows) {
    ...
    for (const p of paths) {
      ...
      // de-dupe session on same path
      const list = byPath.get(p)
      if (!list.some((x) => x.session_id === s.session_id)) list.push(s)
    }
  }
  const out = []
...
export const faceCountsFromFleet = (fleet, deadCount) => {
  const live = fleet?.totals?.live ?? 0
  const clash = buildClash(fleet || { repos: [] }, {}).totals.paths
