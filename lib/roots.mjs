...
//      (read-only — never created/written under legacy; see migrateStateDir).
...
// Legacy on-disk state dir (pre-rename). Read-only fallback — never written
// to by any read verb; only `sage init` / `init --repair` may rename it via
...
// Global config path resolution: new path wins when present; legacy is a
// read-only fallback when only it exists; absent both, WRITES target the new
// path (a user with only a legacy config who runs `sage on` must not end up
// with two disagreeing configs — see lib/control.mjs's setEnabled).
...
export const explainRepoDataDir = ({
  ...
  // Rule 4: built-in default. Rule 5: legacy fallback — read-only, only when
  // the built-in dir has no repos/<id> yet AND the legacy one does (never
  // created/written under legacy; that's migrateStateDir's job, init-only).
