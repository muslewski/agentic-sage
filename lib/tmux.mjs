...
// starttime (jiffies since boot) via /proc/<pid>/stat field 22 — unique per
// process lifetime, so a recycled pid gets a new value. comm (field 2) may hold
// spaces/parens, so slice past the LAST ')' (like ppidOf) and read index 19 of
// the remainder (state=0 … starttime=19). Returned as a string (compare by ===).
export const parseStartTime = (stat) => {
  const after = stat.slice(stat.lastIndexOf(')') + 2).split(' ')
  return after[19] || ''
}

export const startTimeOf = (pid) => {
  try {
    return parseStartTime(fs.readFileSync(`/proc/${pid}/stat`, 'utf8'))
  } catch {
    return ''
  }
}
