...
export const readGuard = (home, repoId) => {
  ...
  return {
    ...
    paths: Array.isArray(g && g.paths) ? g.paths : [],
  }
}
...
export const shouldBlock = (relPath, guard) => {
  if (!guard || guard.enabled !== true) return { blocked: false, matched: null }
...
export const blockMessage = (relPath, matched) =>
  ...
  `another session claimed it. Coordinate before editing. Override: sage guard off`
