...
import { readJson, atomicWriteJson, mergeRecord, isSafeSessionId, readRecord } from './store.mjs'
...
import { doctorLiveJudgeDetail } from './judge-desired.mjs'
import { userScopeWiringChecks } from './user-scope-wiring.mjs'
...
// Link/unlink only mutate an existing open record — never create ghost sessions
// for unknown sids (board pollution + exit-0 lies).
export const linkSession = (home, repoId, sid, state = 'linked') => {
  ...
  if (!cur || typeof cur !== 'object' || Array.isArray(cur)) {
    throw new Error(`no open record for ${sid}`)
  }
  return mergeRecord(home, repoId, sid, { link_state: state, updated_at: new Date().toISOString() })
}
...
export const doctor = (home, cwd) => {
  ...
  // Directory only — a regular file at the home path is not a healthy install.
  const isDir = (p) => {
    try {
  ...
  const newHomeOk = isDir(newHome)
  const legacyHomeOk = !newHomeOk && isDir(legacyHome)
  ...
  checks.push({
    ...
    detail: newHomeOk
      ...
      : legacyHomeOk
        ...
        : exists(newHome) && !newHomeOk
          ? `${newHome} (not a directory)`
          : newHome,
  ...
  // sessions coordinate; sage-judge is the optional live-brief loop; sage-doctor
  // is the /sage-doctor front door.
  ...
  const want = ['sage-fleet', 'sage-doctor', 'sage-judge']
  ...
  const mainRootForAdapter = repoId ? resolveRepoRoot(cwd) : null
  const adapterPath = repoId ? adapterPathFor(home, repoId, mainRootForAdapter) : null
  let adapterDetail = adapterPath ? `present (${adapterPath})` : 'none (core-only — fine)'
  if (!adapterPath && mainRootForAdapter) {
    try {
      if (fs.existsSync(path.join(mainRootForAdapter, 'atlas.config.json'))) {
        adapterDetail =
          'none — atlas vault present; optional: copy memory-atlas examples/with-agentic-sage adapter'
      }
    } catch {
      /* keep default */
    }
  ...
  // Live judge preference (always ok:true — soft policy, not install health fail).
  checks.push({
    name: 'live judge',
    ok: true,
    detail: doctorLiveJudgeDetail(home, { repoId }),
  })

  // User-scope wiring under <home>/.claude — dangling / worktree / nvm-pinned
  // symlinks and settings commands that point at missing files. Read-only;
  // fail-open (never throws). Broken vs latent use different severity marks.
  try {
    for (const row of userScopeWiringChecks(home)) checks.push(row)
  } catch {
    checks.push({
      name: 'user-scope wiring',
      ok: true,
      detail: 'scan skipped (fail-open)',
    })
  }
...
export const renderDoctor = (checks) => {
  ...
  for (const c of checks) {
    // Broken (dangling / wired-missing) → ✗; latent risk (worktree / nvm) → ⚠.
