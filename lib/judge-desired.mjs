 ...
 * Judges are real emitter sessions (`role=judge`), never Agent Status Provider
 * synthetics — so we always pass `noSynthetic: true`. Merging agent-status on
 * every repo re-resolves hundreds of launcher records per call and turns
 * `sage gate` / SessionStart preferred-offline probes into multi-minute hangs
 * on a busy machine (100+ repos × full synthetic merge).
 * @param {string} home
...
export const hasLiveJudgeSession = (home, { now = Date.now() } = {}) => {
  try {
    for (const repoId of listRepoIds(home)) {
      ...
      const sessions = collectSessions(home, repoId, now, { noSynthetic: true })
 ...
 *
 * Brief checks are O(1) file reads; the live-judge scan walks every repo.
 * Try briefs first so a fresh preferred brief (common soft path) never pays
 * the multi-repo walk.
 *
 * @param {string} home
...
export const isJudgeDesireSatisfied = (home, { now = Date.now(), repoId = null } = {}) => {
  ...
  return hasLiveJudgeSession(home, { now })
}
