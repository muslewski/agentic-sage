...
function tokenColor(tok) {
  ...
  if (/^⚔$/u.test(tok)) return 'gold' // live contested count glyph
  if (/^(clear|ghosts|churn)$/.test(tok)) return 'dim' // calm HEAT / band noise rollup
  if (/^(FLEET|ACTIVE|HEAT|CLASH|INVOLVED|MEMORY|ARCHIVE|CLEAN)$/.test(tok)) return 'cream' // panel titles
  if (/^(LIVE|CLASH|MEMORY)$/.test(tok)) return 'cream' // face tabs (active UPPER)
  if (/^(SESSION|STATUS|ZONE|HANDOFF|NAME|BRANCH|AGE)$/.test(tok)) return 'cream' // war column headers
