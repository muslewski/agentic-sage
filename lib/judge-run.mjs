...
import { buildSessionLines } from './about.mjs'
...
export const buildFactBrief = (home, scope, repoId, sid, { now = Date.now() } = {}) => {
  ...
  try {
    ...
    // Auto per-session one-liners for ferry about / navigation (no LLM).
    session_lines = buildSessionLines(fleet, { scope, repoId })
    if (scope === 'fleet') {
...
/**
 * Build argv for the harness CLI.
 *
 * Grok/Claude both take an optional *interactive* initial PROMPT as a
 * positional argument (`grok "…"` / `claude "…"`). Do **not** use
 * `--prompt-file` / `-p` here — those are single-turn and exit immediately
 * (root cause of "harness exited (0)" with an empty TUI).
 *
 * Extra configured args (config.judge.args.*) are prepended.
 *
 * @param {{ name: string, args?: string[] }} harness
 * @param {{ prompt: string, promptPath?: string }} opts
 * @returns {string[]}
 */
export const buildHarnessArgv = (harness, { prompt, promptPath } = {}) => {
  const base = Array.isArray(harness?.args) ? [...harness.args] : []
  const text = typeof prompt === 'string' ? prompt.trim() : ''
  if (!text) return base
  // Interactive initial prompt for both supported harnesses.
  if (harness.name === 'grok' || harness.name === 'claude' || harness.name === 'auto') {
    return [...base, text]
  }
  void promptPath
  return base
...
export const spawnHarnessJudge = (home, { scope, repoId, sid, cwd, harness, takeover }) => {
  ...
  try {
    fs.writeFileSync(
      promptPath,
      `# SAGE live judge\n\n${prompt}\n\n---\nAlso load skill **sage-judge** if available.\n`,
    )
  } catch {
    /* ignore */
  }
  // Pass prompt as interactive initial argv (grok/claude), NOT as --prompt-file
  // (single-turn → empty TUI / instant exit).
  const args = buildHarnessArgv(harness, { prompt, promptPath })
  console.log(
    `sage: starting ${harness.name} as live judge (${scope})\n` +
      `  sid ${sid}\n` +
      `  prompt file: ${promptPath}\n` +
      `  argv: ${harness.bin} <config args…> "<judge prompt>"\n` +
      `  (interactive session — skill sage-judge; publish via sage judge publish)`,
  )
  ...
  return { mode: 'spawn', child, promptPath, args }
}
