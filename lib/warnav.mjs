...
// Re-derive the selection ordinal for the row whose session matches `sid` after
// a model rebuild — so the cursor STAYS on the same session even when rows
// reorder (hot-float), the fleet re-collects, or a filter narrows the list. The
// old code kept a positional ordinal and merely clamped it, so any reshuffle
// silently slid the cursor onto a different session. Falls back to the clamped
// previous ordinal when that session is gone (died / filtered out).
export const reselectBySid = (model, sid, prevOrd = 0) => {
  const idxs = selectableIndices(model)
  if (!idxs.length) return 0
  if (sid) {
    const mi = model.findIndex((m) => !m.isHeader && m.session?.session_id === sid)
    if (mi >= 0) {
      const ord = idxs.indexOf(mi)
      if (ord >= 0) return ord
    }
  }
  return Math.max(0, Math.min(idxs.length - 1, prevOrd))
}
...
// Filter the fleet by a substring query (repo label + displayed session id:
// window_name · branch, matching sessionRow) and/or working-only. Totals stay
// fleet-wide (the stat panels always summarise the whole fleet); only the body
// subset narrows. Repos emptied by the filter drop out.
  ...
  const repos = (fleet.repos || [])
    .map((r) => ({
      ...
      sessions: r.sessions.filter((s) => {
        if (workingOnly && s.liveness !== 'working') return false
        if (!q) return true
        const hay = `${r.label} ${s.window_name || ''} ${s.branch || ''} ${s.session_id || ''}`
...
// Every terminal session across the fleet, flattened. Stamp repo_id from the
// parent fleet repo when the record body lacks it (pre-017 / partial writes) so
// killRecord can resolve sessionFile the same way sage prune does.
